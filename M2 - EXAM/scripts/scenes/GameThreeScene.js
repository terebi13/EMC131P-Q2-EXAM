export default class GameThreeScene extends Phaser.Scene {
    constructor(){
    ...
    preload(){
        this.load.image('tiles3', './assets/maps/sheetC.png');
        this.load.tilemapTiledJSON('tilemap3', './assets/maps/map3.tmj');
        ...
        //backgrounds
        this.load.image('T1', './assets/background/T1.png');
    ...
    create(){
        ...
        let T = this.add.image(this.cameras.main.centerX, this.cameras.main.centerY, 'T1').setScale(3).setScrollFactor(0.3);


        
        this.cameras.main.startFollow(T);
        
        this.cameras.main.on('camerafadeoutcomplete', function () {
            this.cameras.main.fadeIn(2000);
            this.cameras.main.setScroll(0, 0);
            this.cameras.main.setBounds(0, 0, T.width, T.height);
        }, this);
        
        this.cameras.main.setBounds(0, 0, T.width * 2, T.height);
        ...
        this.cameras.main.on('cameraupdate', function () {
            T.x = this.cameras.main.scrollX * 0.1;
        }, this);
        ...
        // skull
        this.enemyGround = this.map.getObjectLayer('jungle enemies')['objects'];
    
        this.gEnemies = this.physics.add.group();
        this.enemyGround.forEach(object => {
            let obj = this.gEnemies.create(object.x, object.y, "skull");
            obj.setScale(.3);
            obj.setScale(object.width/22, object.height/23); 
            obj.setOrigin(0);
    ...
    // Win-Lose Fucntions
    clear(){
        this.scene.start("WinningSceneThree")
        this.sound.stopAll();
    }

    gameOver() {
        this.sound.stopAll();
        this.scene.start('GameOverThreeScene');
      }
